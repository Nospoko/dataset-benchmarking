...
class MidiQuantizer:
    ...
    def quantize_record(self, record: dict) -> dict:
        dstart_bin = np.digitize(record["dstart"], self.dstart_bin_edges) - 1
        duration_bin = np.digitize(record["duration"], self.duration_bin_edges) - 1
        velocity_bin = np.digitize(record["velocity"], self.velocity_bin_edges) - 1
        # Pitch was already quantized
        out = {
            "pitch": record["pitch"],
    ...
    def _build_duration_decoder(self):
        """
        Build a decoder for binned durations.

        This method generates a lookup table where each entry is the mean of two consecutive bin edges from the original duration data.
        The bin edges were calculated by splitting the original dataset (Maestro) into constant-sized bins.
        The last entry is twice the last bin edge, accommodating durations falling into the final bin.
        """
