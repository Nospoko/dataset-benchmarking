...
def plot_piano_roll(piece: ff.MidiPiece, title: str = "Piano Roll") -> plt.Figure:
    ...
    img_path = os.path.join("tmp", "images", f"{title}.png")
...
def convert_to_dstart(record, split: int = None):
    dstarts = np.diff(record["start"])
    dstarts = np.insert(dstarts, 0, 0)
